...
(defun detect-platform-segment (type)
  ...
  (macrolet ((try (segment &optional predicate)
    ...
    (or (try :sdl2)
        (try :out123)
        (try :jack (c org.shirakumo.fraf.mixed.jack present-p))
        #+nx (try :nxau)
        #+bsd (try :oss (probe-file "/dev/dsp"))
        #+windows (try :xaudio2 (c org.shirakumo.fraf.mixed.xaudio2 present-p))
        #+windows (try :wasapi (<= 6 (ldb (byte 8 0) (cffi:foreign-funcall "GetVersion" :int32))))
        #+windows (try :winmm)
        #+linux (try :aaudio)
        #+linux (try :pipewire (c org.shirakumo.fraf.mixed.pipewire present-p))
        #+linux (try :pulse (c org.shirakumo.fraf.mixed.pulse present-p))
        #+linux (try :alsa)
        #+linux (try :oss)
...
(defun construct-input (&key (name :input) (source T) (source-channels 1) (target-channels source-channels) (samplerate (samplerate *server*)) (program-name (name *server*)) device (frames (truncate samplerate 50)))
  (let* ((type (resolve-segment-type 'mixed:source (if (eql T source) (detect-platform-source) source)))
         ...
         (source (if (subtypep type 'mixed:device-source)
                     (make-instance type :pack (mixed:pack unpacker) :name :source :program-name program-name :device device)
                     (make-instance type :pack (mixed:pack unpacker) :name :source :program-name program-name)))
